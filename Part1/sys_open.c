...
asmlinkage long new_sys_open(const char __user *filename,int flags, umode_t mode) {
    kuid_t kuid = current_uid(); 	//get the current user account number (UID) (it comes in a struct)
    if (kuid.val >= 1000)		// if UID the  is a regular user (1000 or over)
        printk(KERN_INFO "User %d is opening file: %s\n", (int)kuid.val, filename);
